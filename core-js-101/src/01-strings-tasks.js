...
function getRectangleString(width, height) {
  ...
  const createLine = (lineFill, left, right) => {
    const line = new Array(width).fill(lineFill);

    line[0] = left;
    line[line.length - 1] = right;

    return line.join('') + LINE_BREAK;
  };

  const topLine = createLine(DASH, BORDER_TOP_LEFT, BORDER_TOP_RIGHT);
  const bottomLine = createLine(DASH, BORDER_BOTTOM_LEFT, BORDER_BOTTOM_RIGHT);
  const sideLine = createLine(SPACE, VERTICAL_LINE, VERTICAL_LINE);
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    result += index >= 0 ? output[index] : str[i];
  }
