...
function chainPromises(array, action) {
  const suitableResults = new Promise((resolve, reject) => {
    const results = [];

    array.forEach((promise) => {
      promise.then((value) => results.push(value)).catch((error) => reject(error));
    });

    resolve(results);
  });

  return suitableResults.then((values) => values.reduce(action));
}
